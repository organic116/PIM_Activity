...
INNER JOIN tb_course ON tb_major.course_id = tb_course.course_id where user_id='$stu_id' ";
		echo "<form name='frmedituser' method='POST' action='admin_editprofilescom.php?stu_id=$stu_id' enctype='multipart/form-data'>";
		...
		if(mysqli_num_rows($result) == 1)
		{
			...
			echo "<select class='custom-select' id='user_prefix' name='user_prefix'>
					<option value='$row[user_prefix]'>$row[user_prefix]</option>
			...
			echo "<input type='text' class='form-control'  id='user_name' name='user_name' value='$row[user_name]' >";
			echo "</div>";
			echo "<div  class='form-group col-md-3'>";
			echo " <label for='user_lastname'>นามสกุล</label>";
			echo "<input type='text' class='form-control'  id='user_lastname' name='user_lastname' value='$row[user_lastname]' >";
			echo "</div>";
			echo "</div>";
			...
			echo "<select class='custom-select'  name='major_id' id='major_id'>";
			$sqlmajor="SELECT DISTINCT
			tb_user.major_id,
			tb_major.major_id,
			tb_major.major_name
			FROM
			tb_user
			INNER JOIN tb_major ON tb_user.major_id = tb_major.major_id";
			$resultmajor=mysqli_query($conn,$sqlmajor) or die ("ไม่สามารถค้นหาข้อมูลได้").mysql_error();
			while ($rowmajor=mysqli_fetch_array($resultmajor)){
				if($rowmajor['major_id'] == $row['major_id'] ){
					$select = "selected";
				} else {
					$select = "";
				}
			echo "<option value='".$rowmajor['major_id']."' ".$select.">".$rowmajor['major_name']."</option>";
			}
			echo "</select>";
			echo "</div>";
			echo "<div  class='form-group col-md-3'>";
			...
			echo "</div>";
			echo "<div  class='form-group col-md-1'>";
			echo " <label for='user_year'>ปีการศึกษา</label>";
			echo "<input type='text' class='form-control'  id='user_year' name='user_year' value='$row[user_year]' readonly>";
			echo "</div>";
			echo "<div  class='form-group col-md-2'>";
			echo "<label for='user_status'>สถานะผู้ใช้</label>";
			echo "<select class='custom-select'  name='user_status' id='user_status'>";
			...
			$resultstatus=mysqli_query($conn,$sqlstatus) or die ("ไม่สามารถค้นหาข้อมูลได้").mysql_error();
			while ($rowstatus=mysqli_fetch_array($resultstatus)){
				if($rowstatus['s_id'] == $row['user_status'] ){
					$select = "selected";
				} else {
					$select = "";
				}
			echo "<option value='".$rowstatus['s_id']."' ".$select.">".$rowstatus['s_name']."</option>";
			}
			echo "</select>";
			echo "</div>";
			echo "<div  class='form-group col-md-2'>";
			...
			if ($row["user_statusqrcode"]=='0') {
					echo "<select class='custom-select' id='user_statusqrcode' name='user_statusqrcode'>
					<option value=$row[user_statusqrcode]>ไม่ได้แจก</option>
					<option value='1'>แจกแล้ว</option>
					</select>";
			}
			else if ($row["user_statusqrcode"]=='1'){
					echo "<select class='custom-select' id='user_statusqrcode' name='user_statusqrcode'>
					<option value=$row[user_statusqrcode]> แจกแล้ว</option>
					<option value='0'>ยังไม่ได้แจก</option>
			...
			echo "</div>";
			echo "<div>";
			echo "&nbsp;&nbsp;&nbsp;&nbsp;<button type='submit' class='btn btn-primary' name='submit' id='submit'>แก้ไขข้อมูลนักศึกษา</button>";
			echo "</div>";
