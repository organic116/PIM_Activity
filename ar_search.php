...
<i class="fas fa-align-justify"></i>
  ...
  <br>
	...
	<table width="100%" cellpadding="4" cellspacing="0" ><tr id="rcorners2">
<td>
<br>
...
<div class="form-group col-md-2.5">
 ...
 <div class="form-group col-md-2.5">
  ...
  &nbsp;&nbsp;&nbsp;&nbsp;<button id='sidebarCollapse' class='btn btn-info' type="submit" name="find_Emp">
		...
		<a href='admin_uishowar.php'>
			   &nbsp;&nbsp;&nbsp;&nbsp;<button type='button' id='sidebarCollapse' class='btn btn-info'>
			 <i class="fas fa-home"></i>&nbsp;กลับหน้าแสดงกิจกรรม
					</button></a>
		<br>
		</br>
		<?php
...
?>
       <?php
	   $fitdate = date("Y-m-d");
$SQL="SELECT * FROM tb_activities  WHERE activities_startdate >= '".$fitdate."'   AND activities_startdate  ";
	if(!$_GET["name_ar"] == ""){
		$SQL .= "AND activities_name LIKE '%".$_GET["name_ar"]."%' ORDER BY activities_addstart DESC";
		//$SQL .= "WHERE activities_startdate >= '".$fitdate."'   AND activities_startdate";
	}
	if(!$_GET["name_selar"] == ""){
		$SQL .= "AND genus_id  = '".$_GET["name_selar"]."' ORDER BY activities_addstart DESC";
		//	$SQL .= "WHERE activities_startdate >= '".$fitdate."'   AND activities_startdate";
	}
	if(!$_GET["name_month"] == ""){
		$SQL .= "AND activities_startdate LIKE '%".$_GET["name_month"]."%' ORDER BY activities_addstart DESC";
	//	$SQL .= "WHERE activities_startdate >= '".$fitdate."'   AND activities_startdate";
...
$rowc = mysqli_fetch_array($result0,MYSQLI_ASSOC);
$counts = $rowc["countjoin"];
$diss = "";
	  if($counts != 0)
	  {
			$diss = "disabled";
...
echo"<td width='100px' bgcolor='#00aae1'><B>ชื่อกิจกรรม</B></td>";
echo"<td width='100px' bgcolor='#00aae1'><B>วันเริ่มกิจกรรม</B></td>";
echo"<td width='100px' bgcolor='#00aae1'><B>เวลาเริ่มกิจกรรม</B></td>";
echo"<td width='30px' bgcolor='#00aae1'><B>ชั่วโมง</B></td>";
echo"<td width='80px' bgcolor='#00aae1'><B>จำนวนผู้เข้าร่วม</B></td>";
echo"<td width='100px' bgcolor='#00aae1'><B>จัดการข้อมูล</B></td>";
echo"</tr>";
echo "<tr align='center' bgcolor='#FFFFF0'>";
echo "<td>$row[activities_name]</td>";
echo "<td>" .'<center>'.DateThai ($row["activities_startdate"])."</td> ";
...
echo "<td>" .'<center>'.$counts.'/'.$row["activities_valueuser"] .  "&nbsp;คน</td> ";
  echo "<td>
			<a href='join_ar.php?ar_id=".$row["activities_id"]."'>
			  <button type='button' id='sidebarCollapse' class='btn btn-success'>
			 <i class='fas fa-info-circle'></i></button></a>

			 <a href='admin_activesedit.php?ar_id=".$row["activities_id"]."'>
			   <button type='button' id='sidebarCollapse' class='btn btn-info'>
			 <i class='fas fa-edit'></i></button></a>

				<a href='del_ar.php?ar_id=".$row["activities_id"]."'>
			<button type='submit' class='btn btn-danger'  name='submit' id='submit' onclick='return confirm('คุณต้องการลบนักศึกษาใช่หรือไม่ ?')  ".$diss."'><i class='fas fa-trash-alt'></i>
			</button></a>
...
$query0 = "SELECT COUNT(activities_id) AS countjoin FROM tb_joinac WHERE activities_id = '".$rowz["activities_id"]."'" or die(mysql_error());
$result0 = mysqli_query($conn,$query0);
$rowc = mysqli_fetch_array($result0,MYSQLI_ASSOC);
$countz = $rowc["countjoin"];
$disz = "";
	  if($countz != 0)
	  {
			$disz = "disabled";
...
echo "<tr align='center' bgcolor='#FFFFF0'>";
echo "<td>$rowz[activities_name]</td>";
echo "<td>" .'<center>'.DateThai ($rowz["activities_startdate"])."</td> ";
echo "<td>".date('H:i',strtotime($rowz["activities_starttime"]))."&nbsp;น. - ".date('H:i',strtotime($rowz["activities_endtime"]))."&nbsp;น.</td> ";
echo "<td>" .'<center>'.$rowz["activities_hour"] .  "</td> ";
echo "<td>" .'<center>'.$countz.'/'.$rowz["activities_valueuser"] .  "&nbsp;คน</td> ";
  echo "<td>
			<a href='join_ar.php?ar_id=".$rowz["activities_id"]."'>
			  <button type='button' id='sidebarCollapse' class='btn btn-success'>
			 <i class='fas fa-info-circle'></i></button></a>

			 <a href='admin_activesedit.php?ar_id=".$rowz["activities_id"]."'>
			   <button type='button' id='sidebarCollapse' class='btn btn-info'>
			 <i class='fas fa-edit'></i></button></a>


				<a href=del_ar.php?ar_id=".$rowz["activities_id"]."'>
			<button type='submit' class='btn btn-danger'  name='submit' id='submit' onclick='return confirm('คุณต้องการลบนักศึกษาใช่หรือไม่ ?')  ".$disz."'><i class='fas fa-trash-alt'></i>
			</button></a>
...
echo "</div>";
	...
	echo "ไม่พบข้อมูลกิจกรรมที่ต้องการค้นหา";
	exit(0);
	}
	mysqli_close($conn);
