...
<i class="fas fa-align-justify"></i>
  ...
  <br>
             <nav class="navbar navbar-expand-lg navbar-light bg-light">
                ...
                <button type="button" id="sidebarCollapse" class="btn btn-info">
                    ...
                    <h4>รายละเอียดการเข้าร่วมกิจกรรม</h4>
    ...
    <table  border='3' align='center' class='form-group table table-bordered' width='100px' style='table-layout:fixed' >
 <tr align='center'>
 ...
 <th width='80px' bgcolor='#6495ED'>ปีที่เข้าศึกษา</th>
 <th width='60px' bgcolor='#6495ED'>ชั้นปี</th>
 </h6></tr>
 </thead>
 <tbody>
 <?php
...
INNER JOIN tb_course ON tb_major.course_id = tb_course.course_id  where user_id='$stu_id'";
 ...
 ?>
 <tr align="center">
 <td  width='20px'><?php echo $row['user_id']; ?></td>
 <td  width='100px'><?php echo $row['user_name']; ?>&nbsp;<?php echo $row['user_lastname']; ?></td>
	<td  width='20px'><?php echo $row['major_name']; ?></td>
	<td  width='20px'><?php echo $row['user_year']; ?></td>
	<td><?php
		 $yer =  date('Y')+543;
...
<?php
   $conn = mysqli_connect('localhost', 'id10717672_organic', 'Tanakorn1340', 'id10717672_dbactivities');
	...
	echo "<table  align='center' border='3' class='form-group table table-bordered'  width='100px' style='table-layout:fixed; width: 700px;' >";
	echo "<tr align='center' bgcolor='FFDAB9'>
 <h6>
 <th width='100px'>ประเภทกิจกรรม</th>
 <th width='100px'>ครั้ง</th>
 <th width='100px'>ชั้วโมง</th>
 </h6></tr>";
	echo  "<tr>";
	echo  "<td  width='100px'><center>จำนวนกิจกรรมบังคับ</center></td>";
	echo  "<td  width='100px'><center>".$rowcount1."</center></td>";
	echo  "<td  width='100px'><center>".$hour1."</center></td>";
	echo  "</tr>";
	...
	echo  "<tr>";
	echo  "<td  width='100'><center>จำนวนกิจกรรมเลือก</center></td>";
	echo  "<td><center>".$rowcount2."</center></td>";
	echo  "<td><center>".$hour2."</center></td>";
	echo  "</tr>";
	...
	echo  "<tr>";
	echo  "<td  width='100'><center>จำนวนกิจกรรมอื่นๆ</center></td>";
	echo  "<td><center>".$rowcount3."</center></td>";
	echo  "<td><center>".$hour3."</center></td>";
	echo  "</tr>";
...
<div class='col-lg-12' >
<table  border='3' align='center' class='form-group table table-bordered'  width='100px' style='table-layout:fixed; width: 700px;'>
<tr align='center' bgcolor='FFDAB9'>
 <h6>
 <th width='100px'>ชื่อกิจกรรม</th>
 <th width='100px'>กลุ่มกิจกรรม</th>
 <th width='100px'>วันที่จัดกิจกรรม</th>
 </h6></tr>
 </thead>
  <?php
$i = 1;
...
INNER JOIN tb_activities_type ON tb_activities.genus_id = tb_activities_type.type_id WHERE activities_id  = '".$aaa ["activities_id"]."' ";
 $querygenus = mysqli_query($con,$sqlgenus);
...
INNER JOIN tb_faculty ON tb_user.faculty_id = tb_faculty.faculty_id where user_id='".$stu_id."' ";
		$result=mysqli_query($conn,$sql) or die ("ไม่สามารถค้นหาข้อมูลได้").mysql_error();
...
$pdf->Cell(0,7,iconv( 'UTF-8','cp874' , 'หลักสูตร : '.$row["course_name"].'') , 0, 1 ,'L',$pdf->Cell(0,7,iconv( 'UTF-8','cp874' , 'คณะ : '.$row["faculty_name"].'' ) , 0, 1,'C' ),$pdf->Cell(0,-7,iconv( 'UTF-8','cp874' , 'พื้นที่ : นนทบุรี ' ) , 0, 1,'R' )  );

$pdf->Cell(0,8,iconv( 'UTF-8','cp874' , 'สาขาวิชา : '.$row['major_name'].'' ) , 0, 1 ,'L');
...
INNER JOIN tb_activities_type ON tb_activities.genus_id = tb_activities_type.type_id WHERE activities_id  = '".$aaa ["activities_id"]."' ";
 ...
 while ($rowgenus = mysqli_fetch_assoc($querygenus) ){
	...
	$pdf->Cell(40,7,iconv( 'UTF-8','TIS-620',''.$rowgenus["activities_year"].'' ),1,0,'C');
	$pdf->Cell(50,7,iconv( 'UTF-8','TIS-620',''.$rowgenus["type_name"].'' ),1,0,'C');
	$pdf->Cell(30,7,iconv( 'UTF-8','TIS-620',''.$rowgenus["activities_hour"].'' ),1,0,'C');
	$pdf->Ln();
	}
	}
	}
